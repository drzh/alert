...
FIELDNAMES = ("obs_time", "intensity_max")
...
def _read_current_record(input_file: Path) -> HistoryRecord:
    ...
    missing_fields = [field for field in FIELDNAMES if not values.get(field)]
    if missing_fields:
        raise ValueError(f"{input_file} is missing required field(s): {', '.join(missing_fields)}")

    obs_time = values["obs_time"]
    _parse_obs_time(obs_time)
    return HistoryRecord(obs_time=obs_time, intensity_max=values["intensity_max"])
...
def _read_history(history_file: Path) -> dict[str, HistoryRecord]:
    ...
    with history_file.open(encoding="utf-8", newline="") as file_obj:
        ...
        if reader.fieldnames is None or any(field not in reader.fieldnames for field in FIELDNAMES):
            return {}
        ...
        for row in reader:
            ...
            try:
                ...
                continue
            records[obs_time] = HistoryRecord(obs_time=obs_time, intensity_max=intensity_max)
...
def _write_history(history_file: Path, records: Iterable[HistoryRecord]) -> None:
    ...
    with tmp_file.open("w", encoding="utf-8", newline="") as file_obj:
        writer = csv.DictWriter(file_obj, fieldnames=FIELDNAMES, delimiter="\t", lineterminator="\n")
        writer.writeheader()
        for record in records:
            writer.writerow({"obs_time": record.obs_time, "intensity_max": record.intensity_max})
